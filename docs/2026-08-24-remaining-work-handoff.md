...
> **2026-08-25 변경**: MSA 서버(192.168.50.30)도 `AUTHZ_PROVIDER=LOCAL` 로 전환했다.
> 그 전까지는 `EXTERNAL`(ST_USER/MySQL) 이라, 통합웹 계정 관리 화면에서 계정을 만들고
> 비밀번호를 설정해도 **로그인이 되지 않았다** — 로그인은 ST_USER 비밀번호로만 검증하고
> 화면이 쓴 값은 `app_user.user_pw` 에만 남기 때문이다. 전환 후에는 화면에서 만든
> 계정·비밀번호가 그대로 통한다. 전환 시 서버의 `admin`·`cerebro-admin` 은 비밀번호가
> 없었으므로, **먼저 로그인 가능한 관리자를 만든 뒤** 모드를 바꿔야 한다(순서가 바뀌면
> 아무도 못 들어간다). `.env` 의 `AUTHZ_PROVIDER` 를 되돌리고 pipeline-api 를 **재생성**하면
> 즉시 원복된다(restart 아님).

- **`ST_USER` → `app_user` 1회 이관 스크립트** — 아직 미작성. LOCAL 전환으로 **시급도가
  올라갔다**: 지금은 사내 사용자가 통합웹에 들어오려면 계정을 수동으로 다시 만들어야 한다
- **EXTERNAL 인증자 코드 제거** — 로컬·서버 모두 `LOCAL` 이라 휴면 상태.
