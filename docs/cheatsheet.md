...
> **통합웹 로그인은 `app_user`(LOCAL) 기준이다.** 로컬·MSA 서버(192.168.50.30) 모두
> `AUTHZ_PROVIDER=LOCAL` 로, 계정 관리 화면에서 만든 계정·비밀번호로 그대로 로그인한다.
> MSA 서버는 2026-08-25 이전까지 `EXTERNAL`(사내 ST_USER/MySQL) 이었다 — 그때는 화면에서
> 계정을 만들어도 로그인이 안 됐다(ST_USER 비밀번호로만 검증). 옛 문서·주석에 "서버는
> EXTERNAL" 로 적힌 곳이 남아 있으면 이 항목이 우선이다.
