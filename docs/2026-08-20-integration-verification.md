...
| **M2-12** | **워크플로우 적재 검증(`verify_landing`)이 타깃 DB가 하나라고 가정 → 실제로는 어느 job도 검증되지 않음 (라이브 확인, 미조치)** | A3 | `etl_job_runtime.py:240-262` — 모든 job이 **고정 연결 하나**(`:254` `postgres_conn_id="target_db_postgres"`)로 조회. `WorkflowCompiler.java:84`가 spec에 담는 것은 `target_tables`(테이블명만, 스키마·연결 없음). 정작 스텝별 커넥션 풀은 `etl_job_step.dbcp_service_id`에 **이미 미러링돼 있는데 spec에도 검증에도 안 쓰인다**. 실측: DW/DZ 10개 job의 load는 `cp-tarantula-192-168-50-12`(`jdbc:postgresql://192.168.50.12:7432/postgres`), csv·log는 `target-postgreDB`(`data-world.net:15432/portal`) — **한 시스템 안에 타깃 DB가 이미 2종**. 그런데 Airflow 연결 `target_db_postgres`는 `docker-compose.yml:584`에서 `target-db:5432/tarantula`(poc 프로파일, 이 환경 미기동)를 가리켜 **셋 중 어느 것도 아니다** | ① 연결이 하나라 job마다 타깃 DB가 다르면 원리적으로 대응 불가 ② 그 하나마저 실제 적재처와 달라 매 실행 `could not translate host name "target-db"`로 실패 ③ `:261`이 예외를 삼키고 «검증 생략»만 출력 → **«검증했고 정상»과 «검증을 못 했다»가 화면·원장에서 구분되지 않는다.** 결과적으로 적재가 0건이어도 워크플로우는 매번 success. 실제 발현: `COM004M`이 `truncate → extract → load` 구성인데 두 번 연속 0행 적재 — 타깃 테이블이 비워진 상태인지 아무도 모른다. (M2-5의 워크플로우판. 옛 팩토리는 Variable 3개 부분설정으로 skip됐고, 여기선 연결 불일치로 skip된다) | **M2** (관측 사각지대. 적재 자체는 정상 동작하므로 데이터 손실은 아님) | ① **최소** — 예외를 삼키지 말고 «검증 안 됨»을 원장·화면에 남긴다(거짓 안심 제거) ② **정공** — 검증을 Airflow가 아니라 **NiFi 안에서** 수행(적재 직후 같은 DBCP로 count). 플랜 B-1 완료 콜백에 건수를 실으면 별도 검증 연결 자체가 불필요 ③ 대안 — spec에 `dbcp_service_id`를 실어 DBCP↔Airflow conn 매핑으로 해당 DB에 접속. 매핑 관리 부담 발생 |
